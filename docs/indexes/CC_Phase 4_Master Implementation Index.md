...
*(ACP-001 through ACP-007 belong to Phase 3 and are recorded in full in the Phase 3 Architecture Record's own registry — not duplicated here to avoid two sources of truth for the same resolutions. This table only tracks ACPs raised during Phase 4 implementation work. ACP-009's and ACP-010's full permanent decision records are maintained separately as `docs/architecture/ACP-009_ProjectStatus_Enum_Reconciliation.md` and `docs/architecture/ACP-010_Retire_Category_Level_Sibling_Paging_After_Gateway.md` respectively — a deliberate documentation choice for cross-phase reconciliations; it does not establish a requirement for future ACPs to use standalone files.)*
