...
- **Data handling:** Obsidian's Metadata Cache API for reading frontmatter (the realized form of WP1's Project Record), and Obsidian's Vault/FileManager API for writes. No custom parser is built — Obsidian's own frontmatter handling is the single source of truth for reading/writing the metadata layer.
...
## ACP Issues Discovered While Defining WP10

**ACP-008 — Metadata Cache timing and the "never cached, always live" derivation rule.**

*Problem:* Phase 3 (via WP3's validation rules, reaffirmed at WP9) requires that derived values — category counts, project summaries, AI observations — be computed live from the Project Record, never cached in a way that could go stale. Obsidian's Metadata Cache API, which this implementation plan relies on for reading frontmatter, is itself an asynchronous cache maintained by Obsidian — it does not guarantee the absolute instant a file is saved is the instant a read reflects that change; there is a small, typically sub-second, propagation delay.

*Why it's a candidate, not a clear defect:* Phase 3's "never cached" rule was written to prevent an *application-level* cache (e.g., a plugin storing its own copy of a count) from silently drifting out of sync with the underlying record — a real risk this rule correctly guards against. Obsidian's own internal metadata cache is a different kind of thing: a platform-level mechanism the entire application depends on, with a bounded, near-instantaneous delay, not an arbitrary staleness window. Whether this counts as "caching" in the sense Phase 3 meant to forbid is genuinely ambiguous from the architecture record alone.

*Proposed resolution:* Treat Obsidian's Metadata Cache as part of the platform's read mechanism, not as an application-level cache subject to the "never cached" rule — i.e., reading through it satisfies "live" for architectural purposes, since there is no additional, separately-introduced staleness beyond what the underlying platform already guarantees.

*Consequences:* If accepted, no design change is needed anywhere in WP1–WP9 — this simply clarifies that the existing rule was never intended to reach platform-level mechanics. If rejected, some additional verification/refresh step would need to be introduced at read time, which the current spec does not describe and which was not anticipated in Section C, D, or E's data-flow definitions.

*Recommendation:* Accept the proposed resolution. This does not appear to be a genuine architectural conflict — it's a reasonable scope clarification that the "never cached" rule governs *application* behavior, not the platform Command Center is built on. Flagging it for your confirmation rather than assuming it silently, consistent with the instruction not to resolve implementation-discovered issues without raising them.

---

WP10 specification complete. Holding here — not beginning WP11 without review and approval.
